import { cn } from "@/lib/utils";

/**
 * The Husky wolf mark, inlined so it can be recoloured.
 *
 * The same geometry as `/public/brand/icon/husky-mark-*.svg`, on the tight viewBox,
 * but painted with `currentColor` instead of a baked fill — an `<img>` cannot be
 * restyled by the page, and this mark needs to change colour as it passes over
 * sections of different tone.
 *
 * Decorative by default. Pass a `title` only if it is ever used as content.
 */
export function HuskyMark({
  className,
  ...props
}: React.ComponentProps<"svg">) {
  return (
    <svg
      viewBox="386 354 308 372"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
      focusable="false"
      className={cn(className)}
      {...props}
    >
      <path d="M561.19,708.43l-42.32-.05-21.55-21.05v-84.74s-30.82-30.26-30.82-30.26l-7.09-24.69,22.05,8.73,8.34,10.67,7.04,9.05,8.14-2.22,9.19,6.94.3-18.74-23.86-23.73-.09-78.32-47.83-55.35-19.65,70.53,6.35,27.08-6.85,20.28,20.45-7.33-27.34,38.51-.04,45.86,49.46,46.88,4.17,25.2-68.6-64.69-.06-59.44,21.95-24.74-15.95-47.65,29.47-103.59,23.88,27.35,45.2,53.21.13,68.69,26.72,26.4-.02,95.8,16.15.1v-95.93s26.73-26.66,26.73-26.66l.04-68.01,69.75-80.8,28.63,104.11-15.65,46.26-7.06-19.81,6.32-26.98-19.28-69.95-48.16,55.36-.05,77.39-23.71,23.75.04,18.82,9.27-6.63c2.73.8,5.97,1.5,8.72,1.12,5.45-5.87,8.95-13.47,15.31-18.22,6.13-4.57,14.3-5.4,21.54-9.34l-7.05,24.67-30.92,30.11v84.73s-21.41,21.29-21.41,21.29ZM547.91,684.13l17.97-19.27-11.77-13.44-27.95.05-12.02,13.52,18.27,19.4,15.5-.27Z" />
      <polygon points="610.48 671.84 614.51 646.65 664.37 599.48 664.38 553.59 636.59 515.32 657.77 522.71 679.42 547.5 679.37 606.94 610.48 671.84" />
    </svg>
  );
}
